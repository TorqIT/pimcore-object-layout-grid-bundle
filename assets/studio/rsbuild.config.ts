...
export default defineConfig({
  ...
  dev: {
    ...(!isDevServer ? { assetPrefix: '/bundles/objectlayoutgrid/studio/build/' + buildId } : {}),
    client: {
      host: 'localhost',
      port: 3032,
      protocol: 'ws'
  ...
  output: {
    manifest: true,
    assetPrefix: '/bundles/objectlayoutgrid/studio/build/' + buildId,
    distPath: {
      root: buildPath
    },
  },
  tools: {
    bundlerChain: (chain, { env }) => {
      chain.output.uniqueName('torq_object_layout_grid_bundle');
    },
  },
  plugins: [
    ...
    pluginModuleFederation({
      name: 'torq_object_layout_grid_bundle',
